...
<head>
  ...
  if (!isset($_GET['select'])) {
    echo '
            <script>
            window.location = "indexentregador.php?select=1";    
            </script>
        ';
  }
  ?>
        ...
        while ($fila = sqlsrv_fetch_array($executar)) {
          ...
          <div class="divResult">
            <form action="alterarentregador.php" method="GET">
              ...
              <table width="100%">
                <tr>
                  <?php echo '<th><span class="spanNome m-l-5">Descrição: ' . $serv_descricao . '</span></th>'; ?>
                  <?php echo '<th><span class="spanNome m-l-5">Por: ' . $user_nome . '</span></th>'; ?>
                </tr>
                <tr>
              ...
              <?php
                if ($serv_status != "5") {
                  ...
                  echo '<input type="hidden" name="status" value="' . $serv_status . '"/>';
                  echo '<button style="width: 50%;" class="login100-form-btn botaoMargem m-t-15 m-b-15" name="id" value="' . $serv_codigo . '">';
